...
def callback_wrapper(base_callback):
  class SelfPlayCallback(base_callback):
    ...
    def _on_step(self) -> bool:
      if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
        ...
        if result and av_reward > self.threshold:
          self.generation += 1
          self.logger.info(f"New best model: {self.generation}\n")
          ...
          #timesteps = (self.base_timesteps + self.num_timesteps) # if reset_num_timesteps is True
          timesteps = self.num_timesteps # if reset_num_timesteps is False
